...
| `consoleFull` | Detailed console output with code snippets and side-by-side blame |
| `ai` | Compact token-efficient output for AI tools (~79% fewer tokens than default) |
| `json` | JSON format output (`jscpd-report.json`) |
| `xml` | XML format output (`jscpd-report.xml`) |
| `csv` | CSV format output (`jscpd-report.csv`) |
| `markdown` | Markdown format output (`jscpd-report.md`) |
| `html` | Interactive HTML report (`jscpd-report.html`) |
| `sarif` | SARIF output for GitHub Code Scanning (`jscpd-sarif.json`) |
| `codeclimate` (alias `gitlab`) | CodeClimate issue format for GitLab Code Quality reports (`gl-code-quality-report.json`) — v5.1.0+ |
| `openmetrics` | Duplication gauges in OpenMetrics text format for GitLab CI metrics reports (`jscpd-metrics.txt`) — v5.1.0+ |
| `badge` | Badge generator for README files |
| `xcode` | Xcode-compatible warnings output |
| `threshold` | Exit with error code when duplication exceeds threshold |
| `silent` | No console output |
| `time` | Output execution time (included in default reporters) |
