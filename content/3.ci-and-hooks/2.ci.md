...
| `history` | Duplication trend over git history: scan every commit in this range (e.g. `v5.0.0..HEAD`) and print a chart and table in the log; needs `fetch-depth: 0` (5.3.0+) | — |
...
Since **5.3.0**, the `history` input scans every commit in a range and prints a bar chart and a per-commit table in the job log, with the change between points and, when `threshold` is set, how far it could be tightened. A scheduled job is the natural home for it:
...
Each point costs one scan; `--history-every` and `--history-limit` (default 30 points) keep long ranges short. See the [history guide](/guides/history) for the output and the options.
