...
seo:
  ...
  description: jscpd --dashboard prints project size, duplication by clone kind, the most duplicated and most complex files, and dead code on one terminal screen.
---
...
::note
Available from jscpd **5.3.0**. The dead-code section covers JavaScript, TypeScript and Python, and Rust when you pass `--rust-diagnostics`.
::
...
| **Dead code** | For the languages the run could read: the share of lines nothing runs, findings per category (`unused-file`, `unused-export`, `unused-symbol`, `unused-import`) and the largest findings |
