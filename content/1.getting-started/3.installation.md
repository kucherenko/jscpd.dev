...
seo:
  description: Install jscpd — a self-contained Rust binary available via curl script, PowerShell, npm, pip, cargo, Homebrew, Nix, or direct download.
  title: Installation
---
...
The [PyPI package](https://pypi.org/project/jscpd/) ships the same prebuilt binary as platform wheels, so Python projects can install jscpd without Node.js:
...
Installs both the `jscpd` and `cpd` commands. For a Node-free [pre-commit](https://pre-commit.com) hook, use `language: python` with `additional_dependencies: ['jscpd==5.2.0']`.
