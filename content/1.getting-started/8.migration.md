...
seo:
  title: Migrating from v4 to v5
  description: Upgrade guide for jscpd v4 to v5 — what changed, CLI flag mapping, config compatibility, and how to stay on v4.
---
...
The v4 (TypeScript) source and documentation live on the [`master-v4` branch](https://github.com/kucherenko/jscpd/tree/master-v4) of the jscpd repository; this site documents v5, plus a [v4 overview page](/getting-started/v4).

> For benchmark comparisons against other tools (jscpd-rs, Duplo, Simian, PMD CPD, and more), see [Detection Speed](/benchmarks/detection-speed).
...
| **MCP server** | Separate `jscpd-server` package | Built in: `jscpd --mcp` |
...
- **Rust crate**: Use [`cpd-finder`](/api/core) in Rust applications
- **MCP**: Use [`jscpd --mcp`](/api/mcp-server) to expose detection to AI assistants
- **Stay on v4**: Keep `jscpd@4` as a project dependency (see [Staying on v4](#staying-on-v4) below)
...
| `jscpd-server` | Built-in MCP server: `jscpd --mcp` (the Node.js server remains on [`master-v4`](https://github.com/kucherenko/jscpd/tree/master-v4)) |
...
v4 is still published on npm under the `latest-4` dist-tag and maintained on the [`master-v4` branch](https://github.com/kucherenko/jscpd/tree/master-v4), with its own README and package docs (summarised on the [v4 page](/getting-started/v4)). If you depend on the Node.js API, the `jscpd-server` package, or the LevelDB/Redis store, you can keep using it:
...
- [ ] Update Node.js API calls to use the CLI (JSON output), the Rust crates, or `jscpd --mcp`
- [ ] Pin `jscpd@4` as a project dependency if you need the Node.js API (see [Staying on v4](#staying-on-v4))