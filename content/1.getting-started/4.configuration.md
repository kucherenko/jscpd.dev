...
| `--complexity` | Print the summary tables ranked by complexity without running clone detection (reporters `console`, `ai`, `json`) (5.3.0+) | `false` |
| `--dashboard` | One screen with the health score, project size, duplication, complexity and dead code; see [Project Dashboard](/guides/dashboard) (5.3.0+) | `false` |
| `--health` | Print only the project health badge: one 0-100 score with a grade, from duplication, dead code and complexity; see [Project Health Score](/guides/health) (5.3.0+) | `false` |
| `--health-input` | JSON file with metrics from other tools (coverage, tests, security) to include in the health score (5.3.0+) | `null` |
| `--kind` | Report only these clone kinds, comma-separated: `exact`, `renamed`, `similar`, `gap`, `ast`; see [Types of Code Clones](/guides/clone-types#reporting-one-kind) (5.3.0+) | all kinds |
| `--history` | Duplication trend over git history: scan every commit in the range (e.g. `v5.0.0..HEAD`) with the same configuration and print a sparkline and a per-commit table with the change between points; with `--threshold`, reports how far it could be tightened | `null` |
| `--history-since` | Select commits since a date (e.g. `2026-01-01`); alone it walks `HEAD`, with `--history` it bounds the range | `null` |
| `--history-every` | Keep every Nth commit of the history series, counted from the newest | `1` |
| `--history-limit` | Maximum number of commits in the history series, sampled evenly with both ends kept | `30` |
...
| `--fail-on-empty` | Exit with code 1 when the scan analyzes no files: the paths exist but nothing matched the `--format`, `--ignore` and `--pattern` filters, or every file was below `--min-tokens`. Without it an empty scan prints a warning and exits 0 | `false` |
...
`--complexity` (5.3.0+) prints the same tables ranked by complexity and skips clone detection, so it is faster on a large tree and has no `DUP%` column. `--summary-top` limits the rows, `--reporters ai` prints one compact line per file and `--reporters json` writes `jscpd-complexity.json`.

```bash
jscpd --complexity --summary-top 3 ./fixtures/summary-demo
```
...
Prose and data files (markdown, reStructuredText, AsciiDoc, text, CSV, JSON, YAML, TOML, INI, lock files) score `0`: an "if" in a README is a word, and `||` in a lock file is a version range.
...
Modes decide what an *exact* clone may differ in. For clones that differ in identifier names or literal values, see [Types of Code Clones](/guides/clone-types).
